...
export class MultimethodCanvasExtractor {
  ...
  static async extractWithFallback(
    ...
    for (let i = 0; i < methods.length; i++) {
      ...
      const methodNumber = i;
      const startTime = Date.now();
      
      try {
        // Record method attempt if imageId is available
        if (imageId) {
          const methodName = attemptedMethods.length === 0 ? 'primary' : `fallback-${i}`;
          debugLogger.recordMethodAttempt(imageId, methodName, methodNumber, startTime);
        }
        
        const result = await method();
        attemptedMethods.push(result.method);

        // Capture DOM state diagnostics after method execution
        const domDiagnostics = await page.evaluate(() => {
          const smartFrame = document.querySelector('smartframe-embed');
          const hasShadowRoot = smartFrame ? !!(smartFrame as any).shadowRoot : false;
          const shadowRoot = hasShadowRoot ? (smartFrame as any).shadowRoot : null;
          const canvas = shadowRoot ? shadowRoot.querySelector('canvas.stage') : null;
          
          return {
            smartFrameExists: !!smartFrame,
            hasShadowRoot,
            canvasExists: !!canvas,
            canvasWidth: canvas ? (canvas as HTMLCanvasElement).width : 0,
            canvasHeight: canvas ? (canvas as HTMLCanvasElement).height : 0
          };
        }).catch(() => ({
          smartFrameExists: false,
          hasShadowRoot: false,
          canvasExists: false,
          canvasWidth: 0,
          canvasHeight: 0
        ...
        // Format DOM state according to expected interface
        const domState = {
          canvasFound: domDiagnostics.canvasExists,
          canvasDimensions: domDiagnostics.canvasExists ? {
            width: domDiagnostics.canvasWidth,
            height: domDiagnostics.canvasHeight
          } : undefined,
          shadowRootFound: domDiagnostics.hasShadowRoot,
          smartframeEmbedFound: domDiagnostics.smartFrameExists,
          selectors: ['smartframe-embed', 'canvas.stage']
        };
        
        // Format extraction details
        const extractionDetails = result.dataUrl ? {
          format: 'image/png',
          size: result.dataUrl.length,
          dimensions: domDiagnostics.canvasExists ? {
            width: domDiagnostics.canvasWidth,
            height: domDiagnostics.canvasHeight
          } : undefined
        } : undefined;
        ...
        if (imageId) {
          debugLogger.recordMethodResult(
            imageId,
            methodNumber,
            result.success,
            result.error,
