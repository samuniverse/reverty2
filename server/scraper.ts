...
class SmartFrameScraper {
  ...
  private async extractImageData(
    ...
    const jobId = config.jobId || 'unknown-job';
    debugLogger.startImageSession(imageId, jobId, url);
    ...
    if (completedImagesTracker.isCompleted(imageId)) {
      ...
      debugLogger.recordOutcome(imageId, false, undefined, undefined, true, 'Already completed');
      debugLogger.endImageSession(imageId);
      ...
      try {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
            ...
            // Record navigation checkpoint
            debugLogger.recordCheckpoint(imageId, 'navigation', {
            ...
            if (httpStatus && httpStatus < 400) {
              ...
              // Record initial wait checkpoint
              debugLogger.recordCheckpoint(imageId, 'initialWait', {
                waitDuration: INITIAL_PAGE_LOAD_WAIT_MS
              });
            }
            ...
            const cookieDismissed = await this.dismissCookieBanner(page);
            debugLogger.recordCheckpoint(imageId, 'cookieBanner', {
              dismissed: cookieDismissed !== false
            });
        ...
        // Record metadata extraction checkpoint
        const fieldsExtracted = Object.keys(metadata).filter(k => metadata[k as keyof typeof metadata] != null);
        const fieldsMissing = Object.keys(metadata).filter(k => metadata[k as keyof typeof metadata] == null);
        debugLogger.recordCheckpoint(imageId, 'metadataExtraction', {
    ...
    if (extractDetails) {
      ...
      // Record metadata normalization checkpoint
      const missingFieldsAfterNorm = [];
      if (!image.titleField) missingFieldsAfterNorm.push('titleField');
      if (!image.subjectField) missingFieldsAfterNorm.push('subjectField');
      if (!image.authors) missingFieldsAfterNorm.push('authors');
      if (!image.dateTaken) missingFieldsAfterNorm.push('dateTaken');
      if (!image.comments) missingFieldsAfterNorm.push('comments');
      
      debugLogger.recordCheckpoint(imageId, 'metadataNormalization', {
        captionGenerated: !!image.comments,
        missingFields: missingFieldsAfterNorm
      });
    }
    ...
    if (canvasExtraction !== "none" && this.canvasExtractor && extractDetails) {
      ...
      debugLogger.recordCheckpoint(imageId, 'shadowRootSetup', {});
    }
    ...
    if (canvasExtraction !== "none" && extractDetails) {
      ...
      debugLogger.recordCheckpoint(imageId, 'cssExpansion', {});
    }
    ...
    if (canvasExtraction === "full" && extractDetails) {
      try {
        ...
        // Record viewport resize checkpoint
        debugLogger.recordCheckpoint(imageId, 'viewportResize', {
          steps: [
            { size: '9990x9990', waitMs: 0 },
            { size: '9999x9999', waitMs: 1000 }
          ]
        });
      } catch (error) {
      ...
      try {
        ...
        // Record viewport resize checkpoint for thumbnail
        debugLogger.recordCheckpoint(imageId, 'viewportResize', {
          steps: [
            { size: `${thumbConfig.width}x${thumbConfig.height}`, waitMs: 0 }
          ]
        });
      } catch (error) {
        console.error(`[${imageId}] ⚠️ Viewport resize failed:`, error instanceof Error ? error.message : error);
    ...
    if (canvasExtraction !== "none" && this.canvasExtractor && extractDetails) {
      try {
        ...
        if (canvasImagePath) {
          ...
          // Record successful canvas extraction checkpoint
          debugLogger.recordCheckpoint(imageId, 'canvasExtraction', {
            mode: canvasExtraction,
            totalAttempts: 1,
            successfulMethod: 'extension',
            methods: []
          });
          
          debugLogger.recordCheckpoint(imageId, 'imageStorage', {
            saved: true,
          ...
          // Record failed canvas extraction checkpoint
          debugLogger.recordCheckpoint(imageId, 'canvasExtraction', {
            mode: canvasExtraction,
            totalAttempts: 1,
            methods: []
        ...
        // Capture failure diagnostics: screenshot and HTML snapshot
        try {
          console.log(`📸 [${imageId}] Capturing failure diagnostics (screenshot + HTML snapshot)...`);
          const screenshotBuffer = await page.screenshot({ encoding: 'binary' }) as Buffer;
          const htmlSnapshot = await page.content();
          await debugLogger.saveDiagnostics(imageId, screenshotBuffer, htmlSnapshot);
        } catch (diagError) {
          console.error(`[${imageId}] Failed to capture diagnostics:`, diagError);
        }
